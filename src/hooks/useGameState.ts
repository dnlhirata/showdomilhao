...
export const useGameState = () => {
  ...
  useEffect(() => {
    ...
    const easyQuestions = shuffledQuestions.filter(q => q.level === 'easy').slice(0, 5);
    const mediumQuestions = shuffledQuestions.filter(q => q.level === 'medium').slice(0, 5);
    const hardQuestions = shuffledQuestions.filter(q => q.level === 'hard').slice(0, 5);
  ...
  const skipQuestion = useCallback(() => {
    if (gameState.gameOver || gameState.skipsLeft <= 0 || !gameState.currentQuestion) return;
  ...
  }, [gameState.gameOver, gameState.skipsLeft, gameState.currentQuestion, gameState.currentQuestionIndex, questions]);

  /**
   * Usa a ajuda dos universitários (apenas uma vez por jogo)
   */
  const useUniversityHelp = useCallback((): UniversityHelpResult | null => {
    if (gameState.gameOver || gameState.universitiesUsed || !gameState.currentQuestion) {
      return null;
    }
    ...
    otherOptions.forEach((index, i) => {
      if (i === otherOptions.length - 1) {
        ...
        const percentage = Math.round(Math.random() * remainingToDistribute * 0.6);
    ...
    return { percentages };
  }, [gameState.gameOver, gameState.universitiesUsed, gameState.currentQuestion]);

  /**
   * Usa as cartas (elimina alternativas incorretas)
   */
  const useCardsHelp = useCallback((cardValue: CardValue) => {
    if (gameState.gameOver || gameState.cardsUsed || !gameState.currentQuestion) {
      return;
    }
   ...
   */
  const isOptionHidden = useCallback((optionIndex: number) => {
